...
async function getNintendoAccountSessionToken(dm) {
    ...
    const filter = (m) => m.content.startsWith("npf71b963c1b7b6d119://auth");
    let authUrlMessage = await dm.awaitMessages({ filter, max: 1, time: 600000, errors: ['time'] }).catch(async (error) => {
...
;
async function statsCommand(dm) {
    let nxapi = await import('nxapi');
    nxapi.addUserAgent('ccastatsbot/1.0.0 (+https://github.com/Candygoblen123/CCAStatsBot)');
    let welcomeMessage = await dm.send(`Thank you for registering with the CCA Stats Team!
...
If you wish to continue send \`continue\` Otherwise send anything else`);
    if (await dm.awaitMessages({ max: 1, time: 15000, errors: ['time'] })
        .then(collected => {
    ...
    const teamName = await dm.awaitMessages({ max: 1, time: 600000, errors: ['time'] }).catch(async (error) => {
        await dm.send("Response timeout. Please start the process over.");
        throw error;
    });
    await dm.send('please confirm by entering your teamname again');
    const teamNameConfirm = await dm.awaitMessages({ max: 1, time: 600000, errors: ['time'] }).catch(async (error) => {
        await dm.send("Response timeout. Please start the process over.");
        throw error;
    });
    ...
    const filter = (m) => m.content.startsWith("select");
    let selectionMessage = await dm.awaitMessages({ filter, max: 1, time: 600000, errors: ['time'] }).catch(async (error) => {
        await dm.send("Response timeout. Please start the process over.");
        throw error;
    });
    ...
    while (true) {
        ...
        selectionMessage = await dm.awaitMessages({ filter, max: 1, time: 600000, errors: ['time'] }).catch(async (error) => {
            await dm.send("Response timeout. Please start the process over.");
            throw error;
        });
    ...
    fs.access(filePath, fs.constants.F_OK, (err) => {
        if (err) {
            ...
            writeStream.write(header + csvString, 'utf-8', (err) => {
                if (err) {
            ...
            const writeStream = fs.createWriteStream(filePath, { flags: "a" });
            writeStream.write(csvString, 'utf-8', (err) => {
                if (err) {
                    console.error("Appending Data Failed!", err);
                }
            });
    ...
    //        }
    //    })
    await dm.send({ content: "Your data has been sent!, You can verify what players we have recieved in the csv!",
        files: [{ attachment: buffer, name: "data.csv" }]
    });
    function extractRangeFromString(inputString) {
