import { LinkedFieldType, OperationFieldType } from '../../generated-type-helpers.js';
import { CoopBossResult, CoopEnemy, CoopGrade, CoopGroupHighestResult, CoopHistoryDetail, CoopHistoryDetailConnection, CoopHistoryGroup, CoopHistoryGroupConnection, CoopPlayerResult, CoopPointCard, CoopResult, CoopScale, CoopStage, CoopSupplyWeapon, CoopWaveResult, Image } from '../../types.js';
/**
 * 50be9b694c7c6b99b7a383e494ec5258 refetchableCoopHistory_coopResultQuery
 *
 * query refetchableCoopHistory_coopResultQuery
 *
 * Generated 10/06/2023 from SplatNet 3 4.0.0-d5178440.
 */
interface refetchableCoopHistory_coopResultQuery_50be9b6 {
    coopResult: OperationFieldType<'refetchableCoopHistory_coopResultQuery', 'coopResult', {
        historyGroups: LinkedFieldType<CoopResult, 'historyGroups', {
            nodes: LinkedFieldType<CoopHistoryGroupConnection, 'nodes', {
                endTime: CoopHistoryGroup['endTime'];
                highestResult: LinkedFieldType<CoopHistoryGroup, 'highestResult', {
                    grade: LinkedFieldType<CoopGroupHighestResult, 'grade', {
                        id: CoopGrade['id'];
                        name: CoopGrade['name'];
                    }, false>;
                    gradePoint: CoopGroupHighestResult['gradePoint'];
                    jobScore: CoopGroupHighestResult['jobScore'];
                    trophy: CoopGroupHighestResult['trophy'];
                }, false>;
                historyDetails: LinkedFieldType<CoopHistoryGroup, 'historyDetails', {
                    nodes: LinkedFieldType<CoopHistoryDetailConnection, 'nodes', {
                        id: CoopHistoryDetail['id'];
                        afterGrade: LinkedFieldType<CoopHistoryDetail, 'afterGrade', {
                            id: CoopGrade['id'];
                            name: CoopGrade['name'];
                        }, false>;
                        afterGradePoint: CoopHistoryDetail['afterGradePoint'];
                        bossResult: LinkedFieldType<CoopHistoryDetail, 'bossResult', {
                            boss: LinkedFieldType<CoopBossResult, 'boss', {
                                id: CoopEnemy['id'];
                                name: CoopEnemy['name'];
                            }, false>;
                            hasDefeatBoss: CoopBossResult['hasDefeatBoss'];
                        }, false>;
                        coopStage: LinkedFieldType<CoopHistoryDetail, 'coopStage', {
                            id: CoopStage['id'];
                            name: CoopStage['name'];
                        }, false>;
                        gradePointDiff: CoopHistoryDetail['gradePointDiff'];
                        memberResults: LinkedFieldType<CoopHistoryDetail, 'memberResults', {
                            deliverCount: CoopPlayerResult['deliverCount'];
                        }, true>;
                        myResult: LinkedFieldType<CoopHistoryDetail, 'myResult', {
                            deliverCount: CoopPlayerResult['deliverCount'];
                        }, false>;
                        nextHistoryDetail: LinkedFieldType<CoopHistoryDetail, 'nextHistoryDetail', {
                            id: CoopHistoryDetail['id'];
                        }, false>;
                        previousHistoryDetail: LinkedFieldType<CoopHistoryDetail, 'previousHistoryDetail', {
                            id: CoopHistoryDetail['id'];
                        }, false>;
                        resultWave: CoopHistoryDetail['resultWave'];
                        waveResults: LinkedFieldType<CoopHistoryDetail, 'waveResults', {
                            teamDeliverCount: CoopWaveResult['teamDeliverCount'];
                        }, true>;
                        weapons: LinkedFieldType<CoopHistoryDetail, 'weapons', {
                            image: LinkedFieldType<CoopSupplyWeapon, 'image', {
                                url: Image['url'];
                            }, false>;
                            name: CoopSupplyWeapon['name'];
                        }, true>;
                    }, true>;
                }, false>;
                mode: CoopHistoryGroup['mode'];
                rule: CoopHistoryGroup['rule'];
                startTime: CoopHistoryGroup['startTime'];
            }, true>;
        }, false>;
        historyGroupsOnlyFirst: LinkedFieldType<CoopResult, 'historyGroups', {
            nodes: LinkedFieldType<CoopHistoryGroupConnection, 'nodes', {
                historyDetails: LinkedFieldType<CoopHistoryGroup, 'historyDetails', {
                    nodes: LinkedFieldType<CoopHistoryDetailConnection, 'nodes', {
                        __typename: 'CoopHistoryDetail';
                        id: CoopHistoryDetail['id'];
                    }, true>;
                }, false>;
            }, true>;
        }, false>;
        monthlyGear: LinkedFieldType<CoopResult, 'monthlyGear', {
            __typename: string;
            image: LinkedFieldType<CoopResult['monthlyGear'], 'image', {
                url: Image['url'];
            }, false>;
            name: CoopResult['monthlyGear']['name'];
        }, false>;
        pointCard: LinkedFieldType<CoopResult, 'pointCard', {
            defeatBossCount: CoopPointCard['defeatBossCount'];
            deliverCount: CoopPointCard['deliverCount'];
            goldenDeliverCount: CoopPointCard['goldenDeliverCount'];
            limitedPoint: CoopPointCard['limitedPoint'];
            playCount: CoopPointCard['playCount'];
            regularPoint: CoopPointCard['regularPoint'];
            rescueCount: CoopPointCard['rescueCount'];
            totalPoint: CoopPointCard['totalPoint'];
        }, false>;
        regularAverageClearWave: CoopResult['regularAverageClearWave'];
        regularGrade: LinkedFieldType<CoopResult, 'regularGrade', {
            id: CoopGrade['id'];
            name: CoopGrade['name'];
        }, false>;
        regularGradePoint: CoopResult['regularGradePoint'];
        scale: LinkedFieldType<CoopResult, 'scale', {
            bronze: CoopScale['bronze'];
            gold: CoopScale['gold'];
            silver: CoopScale['silver'];
        }, false>;
    }>;
}
export default refetchableCoopHistory_coopResultQuery_50be9b6;
export { refetchableCoopHistory_coopResultQuery_50be9b6 };
