import { LinkedFieldType, OperationFieldType } from '../../generated-type-helpers.js';
import { Color, CurrentPlayer, Fest, FestConnection, FestPlayerResult, FestTeam, FestTeamResult, Image } from '../../types.js';
/**
 * 73b9837d0e4dd29bfa2f1a7d7ee0814a FestRecordRefetchQuery
 *
 * query FestRecordRefetchQuery
 *
 * Generated 30/08/2023 from SplatNet 3 4.0.0-22ddb0fd.
 */
interface FestRecordRefetchQuery_73b9837 {
    currentPlayer: OperationFieldType<'FestRecordRefetchQuery', 'currentPlayer', {
        name: CurrentPlayer['name'];
        userIcon: LinkedFieldType<CurrentPlayer, 'userIcon', {
            url: Image['url'];
        }, false>;
    }>;
    festRecords: OperationFieldType<'FestRecordRefetchQuery', 'festRecords', {
        nodes: LinkedFieldType<FestConnection, 'nodes', {
            id: Fest['id'];
            endTime: Fest['endTime'];
            image: LinkedFieldType<Fest, 'image', {
                url: Image['url'];
            }, false>;
            lang: Fest['lang'];
            myTeam: LinkedFieldType<Fest, 'myTeam', {
                id: FestTeam['id'];
                color: LinkedFieldType<FestTeam, 'color', {
                    r: Color['r'];
                    g: Color['g'];
                    b: Color['b'];
                    a: Color['a'];
                }, false>;
                image: LinkedFieldType<FestTeam, 'image', {
                    url: Image['url'];
                }, false>;
                teamName: FestTeam['teamName'];
            }, false>;
            playerResult: LinkedFieldType<Fest, 'playerResult', {
                grade: FestPlayerResult['grade'];
            }, false>;
            startTime: Fest['startTime'];
            state: Fest['state'];
            teams: LinkedFieldType<Fest, 'teams', {
                id: FestTeam['id'];
                result: LinkedFieldType<FestTeam, 'result', {
                    isWinner: FestTeamResult['isWinner'];
                }, false>;
                teamName: FestTeam['teamName'];
            }, true>;
            title: Fest['title'];
        }, true>;
    }>;
}
export default FestRecordRefetchQuery_73b9837;
export { FestRecordRefetchQuery_73b9837 };
